...
//
// Phase 188 Plan 03 (PROV-01): getEstimateByPublicToken() (lib/queries/share.ts)
// now resolves attached-photo signed URLs via serverStorage() ->
// assertServer(), which throws whenever `typeof window !== 'undefined'`.
// The suite's global environment is jsdom (vitest.config.ts), where `window`
// always exists, so this spuriously trips here unless this file opts into
// the `node` environment (same fix as tests/unit/share-query.test.ts and the
// other files this plan touched for the same reason).
