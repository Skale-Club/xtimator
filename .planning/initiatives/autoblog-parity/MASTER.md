...
| Código passar no typecheck e nos testes e não CONSTRUIR | Correr o build de cada repo antes de dar por feito. No xmartmenu, três páginas novas resolviam todas para `/blog` — os grupos de rotas do Next não entram no URL — e o `next build` recusava-se a construir, com 105 testes verdes e zero erros de tsc. Nenhuma das duas ferramentas constrói rotas |
