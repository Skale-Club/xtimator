...
## Last Milestone: v1.9 Custom Domain Support ✅ (shipped 2026-05-10)

Custom domain per empresa — owners configuram `estimates.mycompany.com`, proxy.ts detecta o host e passa `whiteLabelMode` para o EstimateView ocultar o footer do Xtimator. 2 phases, 2 plans.

## Current Milestone: v2.0 WhatsApp Estimate Channel

**Goal:** O dono da empresa nunca precisa abrir o Xtimator para gerar um orçamento — manda áudio, texto ou foto pelo WhatsApp, confirma, e o bot envia o orçamento ao cliente.

**Target features:**
- Webhook infrastructure — `POST /api/webhooks/whatsapp` with Meta HMAC-SHA256 signature verification, phone→company routing
- Phone number registration — `/settings/integrations` WhatsApp card, connect + verify flow
- Inbound processing — audio/text/photo received via WhatsApp, routed through Whisper + Claude Vision + generate-estimate pipeline
- Confirmation flow — bot sends estimate summary, owner replies "send" / "edit X" / "cancel", session expires in 30min
- Outbound delivery — sends estimate to client as formatted text, PDF attachment, or share link (configurable per company)

## Current State

**Version:** v2.0 WhatsApp Estimate Channel — 🔲 IN PROGRESS
**Phases complete:** 39/39 (v1.9 complete) | **Build:** passing
**Last shipped:** Phase 39 — Subdomain Routing + White-label (2026-05-10)
