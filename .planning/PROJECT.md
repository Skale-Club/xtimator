...
## Current Milestone: v4.17 Admin Polish & Credit UX Compliance 🚧 (started 2026-07-06)
...
**Target features:**
- **Tenant Plans page credit-leak fix + accuracy (highest priority — repairs a violated locked decision)** — eliminate every raw credit-count leak found on `/settings/billing` (`TopUpPackCard`'s "≈ X credits" subtext, `AutoTopupDialog`'s pack-picker showing credits, `CreditHistoryList`'s per-row credit deltas); add a real visual progress bar to the topbar `CreditChip` (today text-only "X% used"); reconcile `TierCardsGrid`'s hardcoded tier feature/price copy against `billing_config` reality.
- **Admin nav reorganization** — reorder to Dashboard, Companies, Inbox first; introduce a new "Content" group (a new sidebar UI pattern — none exists today) containing Landing Page, Pages, Blog, SEO, Branding; rename "Legal Pages" → "Pages" including the slug (`/admin/legal` → `/admin/pages`).
- **Naming fixes for two owner-flagged confusing labels** — tenant Settings sidebar "Message" → "Message Template" (matches the page's own internal card title already); super-admin "Support Mode" → "View as Company" (button, banner, exit-button copy only).
- **Admin Billing page overhaul** — replace the tier/MRR-centric admin `/admin/billing` view (hardcoded `pro*29 + business*99` MRR math, force-tier/grant-credits as the only actions) with a credit-model-centric view built on the already-shipped credit ledger + cost-visibility stack (`getCreditOverview`, `getCompanyCostOverview`, `aggregateAiCostByOperation`) — per-company credit balance, real AI cost vs markup, auto-top-up status — keeping force-tier/grant as secondary actions, not the focus.
- **Inbox visual redesign ("Premium Xtimator" direction)** — apply the existing Phase-71 glassmorphism tokens/components (`--glass-*` CSS vars, `Card variant="glass"`) to the v4.16 two-pane conversation viewer: deterministic-color initials avatars, colored unread accent bars/dots, glass-surface rows, richer hover/selected states — replacing the current plain, avatar-less, flat-gray implementation.

**Key context (locked):**
- No new backend/ledger logic in this milestone — 100% reuse of the existing credit-ledger/billing_config/cost-aggregation functions shipped in v4.7 (110-116) and v4.15 (150-153); this is UI/UX polish + a compliance bugfix, not new billing logic.
- The tenant credit-leak fix is the single highest-priority item — it repairs an already-violated locked decision (CREDITUI-04 from v4.15: tenants must NEVER see a raw credit count or $ figure) — ships first, independent of everything else.
- User-facing renames only; internal naming stays put, mirroring the established WhatsApp→Inbox precedent: Legal Pages' public routes (`/privacy-policy`, `/terms-of-service`) and the `legal_pages` DB table are untouched — only the admin nav label + `/admin/legal` route rename to "Pages"/`/admin/pages`. Support Mode's internal functions (`startSupportSession`, etc.), audit-log action literals (`company.support_mode_start/end`), and cookie name are untouched — only user-facing button/banner copy changes to "View as Company".
- The "Content" sidebar group is a brand-new UI pattern (today's nav is a flat unordered list with zero grouping) — this phase builds the grouping component, not just reorders items within an existing one.
- Inbox design direction is "reuse Xtimator's own Phase-71 glass tokens/components" (not a literal Xphere clone) — a deterministic name→color initials-avatar utility needs to be built (none exists yet; the only precedent, `admin-list.tsx`, uses plain gray initials).
- Executed fully autonomously overnight per the standing no-checkpoint-interruptions preference (the owner is asleep and explicitly asked for autonomous completion by morning); where the owner wasn't reachable for a naming call (Support Mode, Message), best-judgment names were chosen and the rationale is documented here and in the phase CONTEXT docs.
