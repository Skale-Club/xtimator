...
## Last Milestone: v1.8 Iterative Estimate Refinement ✅ (shipped 2026-05-09)

Text/voice/photo refinement panel in estimate editor — users can instruct the AI to modify the current estimate without regenerating from scratch. 3 phases, 3 plans. All refinement requirements validated.

## Current Milestone: v1.9 Custom Domain Support

**Goal:** Empresas podem servir os orçamentos compartilhados a partir do seu próprio domínio (ex: `estimates.mycompany.com`) em vez de `xtimator.com/estimate/{token}` — branding 100% da empresa no link enviado ao cliente.

**Target features:**
- `custom_domain` column on companies table — per-company domain configuration
- Subdomain routing in `proxy.ts` — custom host maps to `/estimate/{token}` without revealing Xtimator
- White-label mode in `EstimateView` — footer "Generated by Xtimator" hidden when custom domain is active
- DNS/CNAME setup guide in settings — owner configures their domain; Vercel manages TLS automatically

## Current State

**Version:** v1.9 Custom Domain Support — 🔲 IN PROGRESS
**Phases complete:** 37/37 (v1.8 complete) | **Build:** passing
...
- **Seeds harvested:** SEED-001 (i18n → v1.2), SEED-002 (brand identity → v1.2), SEED-003 (price book → v1.3), SEED-004 (plain-text estimate → v1.4), SEED-005 (multi-modal input → v1.5/v1.6), SEED-006 (iterative refinement → v1.8), SEED-007 (frictionless client → v1.5/v1.7). SEED-009 (custom domain → v1.9 — active).
- **Seeds dormant:** SEED-008 (WhatsApp channel — awaiting integrations milestone).
...
---
*Last updated: 2026-05-10 — v1.9 Custom Domain Support milestone started*
