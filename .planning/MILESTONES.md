...
**Goal (delivered):** Replace the raw numeric credit counter with a Claude-Console-style usage progress bar (tenants see only a % consumed, never $/credit math), move exact $ cost visibility to a super-admin-only surface, rework the top-up purchase flow to configurable dollar packs with optional auto-top-up, and give the super admin an audited way to enter a tenant's live app view for support plus a properly paginated/searchable/filterable Companies admin screen. Source: [SEED-039](seeds/SEED-039-usage-progress-bar-dollar-topup.md) + [SEED-040](seeds/SEED-040-super-admin-tenant-impersonation-companies-overhaul.md).

**What shipped, phase by phase:**

- **Companies Admin Screen Overhaul (150)** (ADMINCO-01..04) — `app/admin/companies/page.tsx` rebuilt on a `searchParams`-driven server-side search (name via `ilike`, email via `auth.admin.listUsers()` → `company_members` resolution) + combinable filters (tier, AI-override presence, demo-vs-real) + `.range()` pagination with `{count:'exact'}` and a visible total, mirroring the existing Phase-93 Event Log pattern verbatim. The pre-existing "Demo Accounts" grouping, `HandoffButton`, and "Configure →" actions kept working unchanged (regression-tested).
- **Super-Admin Support Mode / Tenant Impersonation (151)** (SUPPORT-01..04) — `lib/auth/support-mode.ts`: a signed, time-boxed "acting-as-company" session claim (HMAC-signed cookie, re-verifies `platform_admins` on every read — never trusts the cookie's claim alone), NOT a real Supabase identity switch. `app/(app)/layout.tsx` gained a Support Mode branch (checked before the tenant's own `getActiveCompany()`) rendering a persistent `SupportModeBanner`, suppressing the company switcher. Every session (start/end, company, admin, duration) is audit-logged via the existing `lib/admin/audit-log.ts`. Entry point: a "Support Mode →" row action on Phase 150's Companies list, visually distinct from `HandoffButton` (a different, pre-existing feature: demo-account-to-prospect owner invite — not merged or conflated).
- **Usage Progress Bar + Super-Admin Cost Visibility (152)** (CREDITUI-03..05) — `components/billing/credit-balance-card.tsx` and `components/app-shell/credit-chip.tsx` rewritten to `percentUsed`-only props (the raw credit balance no longer reaches either component); a shared `lib/billing/usage-percent.ts` computes the color-escalating (green/amber/red) percentage server-side. A static neutrality test locks the tenant-facing boundary: no `real_cost_usd`/`markup`/`balance_after` may appear anywhere in `lib/queries/credits.ts` or `components/billing/**`. A new super-admin-only `CompanyCostCard` on the company detail admin page shows exact balance/real cost/markup per company via a genuinely separate query (`lib/queries/admin-company-cost.ts`), never reachable from any tenant route. One gap found by verification and closed same-phase: a bonus-credit notification was still showing a raw number to tenants — reworded to qualitative copy.
- **Dollar-Pack Top-Up + Auto-Top-Up (153)** (CREDITUI-06, CREDITUI-07) — `billing_config.topUpPacks` changed to 3 dollar-denominated packs ($20/$50/$100) with a new `TopUpPackCard`/`TopUpPacksGrid` picker (mirrors the existing tier-card pattern), reusing the pre-existing Phase-113 one-time Stripe checkout route unmodified. Auto-top-up (the milestone's highest-risk piece — real off-session Stripe charges) shipped behind a platform-wide `billing_config.autoTopupEnabled` kill switch (default `false`) AND each tenant's own opt-in: an atomic Postgres-RPC in-flight lock (`acquire_autotopup_lock`/`release_autotopup_lock`) guarantees exactly one charge fires when concurrent debits race the same company's threshold crossing (proven by a dedicated concurrency test), a `mode:'setup'` Stripe Checkout captures the saved payment method, and the settings-save action independently re-verifies server-side that a payment method exists before ever persisting `auto_topup_enabled:true`. This phase's execution was paused mid-flight by the Claude Code permission classifier (building automatic off-session charging is a high-severity financial-automation action) and resumed only after explicit, specifically-contextualized user authorization via a structured confirmation.

**Architecture invariants honored:** no new credit ledger — the `credit_ledger`, markup math, and low-balance logic from SEED-035/CREDITUI-01/02 (Phase 115) were reused exactly as-is; this milestone is a UI + purchase-flow layer on top. Tenants never see a raw credit count or a $ figure anywhere (enforced by a static test, not just convention). Support Mode is provably not a real identity switch (signed claim + live re-verification + audit log + auto-revocation), and is architecturally distinct from `HandoffButton`. Every new billing number (pack prices, auto-top-up kill switch) lives in `billing_config`, never hardcoded.

**Milestone significance:** closes out both halves of the seeds it was harvested from — the tenant-facing credit UX now matches the Anthropic Console pattern the owner referenced, and the super admin gained both an operationally necessary support tool (Support Mode) and a scalable Companies list. The auto-top-up capability is the first automatic-charging code in the codebase and required a distinct, explicit authorization step beyond the milestone's general "execute autonomously" mandate — a deliberate, non-bypassed safety gate.

**Operational deferrals (carry-forward):** apply migration `20260705000002_phase153_auto_topup_columns.sql` to remote (CI→GHCR→Coolify, or explicit MCP apply with owner go-ahead); add an admin-panel UI toggle for `billing_config.autoTopupEnabled` (currently requires a direct `platform_integrations` metadata edit to flip on); live UAT of the 3 Phase-151 items flagged for human verification (visual banner rendering, live audit-log inspection, error-toast UX) and of the full auto-top-up charge flow against a real Stripe test-mode card.

## v4.12 Team Seats & Member Invites (Shipped: 2026-06-25)

**Phases completed:** 6 (135, 136, 137, 138, 139, 140) · full unit suite green (~2552 passing; the only failures are the documented Windows parallel-import flakes that pass in isolation)

**Goal (delivered):** Turn the dormant `company_members` foundation (Phase 79) into a real team-seats feature — invite teammates into the SAME organization (`company`), assign `owner`/`admin`/`member` roles with server-side authority, and bill per seat at a price that is FULLY configurable in the super-admin `billing_config` panel (nothing hardcoded). The costly multi-tenancy work was already paid down (all RLS authorizes via `company_members`), so this was a PRODUCT + BILLING surface, not a data-model rebuild: invite/accept/revoke/remove/role flows + a `Settings → Team` UI + per-seat billing whose price and per-tier included-seat counts are 100% operator-configurable, riding the existing Stripe subscription as a quantity item, gated by `enforcementEnabled` (calibrate before charging). Every single-owner org stays byte-for-byte unchanged (retrocompat). Source: [SEED-037](seeds/SEED-037-team-seats-member-invites.md).

**What shipped, phase by phase:**

- **Schema + Roles + Authorization (135)** (SEAT-01, SEAT-02) — one idempotent authored-only migration `20260628000001` widening the `company_members.role` CHECK to `('owner','admin','member')` (DROP/ADD the named CHECK) + creating `company_invites` (`token`/`status`/`expires_at`/`role` `admin|member`) with owner/admin-manage RLS mirroring the Phase-79 posture. The single server-side `requireCompanyRole`/`requireCompanyManager`/`requireCompanyOwner` gate (RLS-bound, typed errors, NEVER client-trusted) — the SOLE role authority every team + billing action funnels through. Ships dormant — existing `owner` rows untouched, no `companies` billing-column change.
- **Invite Lifecycle + Email (136)** (SEAT-03) — `inviteMember`/`revokeInvite` gated by `requireCompanyManager`; a `randomBytes` base64url single-use token with a 7-day expiry; a Resend invite email carrying an ABSOLUTE `/invite/accept` link. Rejects non-manager / invalid-email / `owner`-role / already-member / duplicate-pending. The token is NEVER returned to the client (delivered only via the email). A pending invite does not consume a billable seat.
- **Accept Onboarding (137)** (SEAT-04) — `acceptInvite(token)` takes token-authority via the SERVICE client: an atomic single-use `status` flip BEFORE the member insert, with expiry + email-match checks, idempotent membership, then `switchActiveCompany`. The `/invite/accept` route handles both authed and unauthed visitors; the invited new-user signup SKIPS company creation and JOINS the existing company — achieved by ROUTING (the route lives outside the `(app)` group + a guarded `?next`), so the always-creates-a-company onboarding is bypassed cleanly.
- **Member Management UI (138)** (SEAT-05) — `removeMember`/`changeMemberRole` gated (last-owner + owner-target + role-enum guards) + `listCompanyRoster`; the `Settings → Team` page + `TeamSection` (members + pending-invites, with invite/remove/role controls for owner/admin and a read-only view for members); mobile-safe; runtime i18n. The legacy Staff tab is retained (a separate mechanism — not folded in).
- **Configurable Seat Billing (139)** (SEAT-06, SEAT-07) — `billing_config` gains `seatPriceCents` (global) + `tiers[tier].includedSeats` (per-tier) as calibration placeholders, editable in the super-admin panel with NOTHING hardcoded; the deep-merge reader tolerates rows written before the fields existed. Pure `computeBillableSeats = max(0, active − included)` / `computeSeatChargeCents`; `syncSeatBilling(companyId)` gated by `enforcementEnabled` (record-only until calibrated), idempotent, never-throws, writing a Stripe seat-quantity item whose `unit_amount` comes from config (NO hardcoded Price ID), wired into the `acceptInvite`/`removeMember`/`changeMemberRole` success paths (a billing failure never rolls back membership). Retrocompat: a single owner within `includedSeats` → zero billable seats, no Stripe write.
- **Seat-Cost Transparency UI (140)** (SEAT-08) — `buildSeatCostSummary` (pure, server-side, reusing the Phase-139 seat functions, reading `getBillingConfig`) → the `Settings → Team` surface shows the active seat count + the per-seat price + the projected monthly cost (owner/admin-gated, mobile-safe, runtime i18n), with a truthful "not yet active" note while `enforcementEnabled` is false. Nothing hardcoded — same transparency principle as the 1%-fee disclosure.

**Architecture invariants honored:** the org unit is `company`; a seat = one `company_members` row — NO new "organization" entity, and the EXISTING `company_members`-based RLS already authorizes every org-owned surface (clients, price book, estimates, credits, Connect payout), so a second member reads it for free — reused, NOT rebuilt. Roles are exactly `owner`/`admin`/`member` (`viewer` deferred to v2); exactly one `owner` per company; authority is SERVER-SIDE only (`requireCompanyRole` + RLS), never client-trusted, and lives in ONE place. ZERO hardcoded billing numbers — the seat price + per-tier included-seat counts live in `billing_config`, read via `getBillingConfig()`, editable in the super-admin panel (operators-only; tenants never see it), applied without a deploy. Billable seats = `max(0, active − included)`; the charge rides the existing platform subscription (`companies.stripe_subscription_id`) as a quantity item, re-synced on membership change, GATED by `enforcementEnabled` (calibrate before charging). A pending invite does NOT consume a billable seat (counted on acceptance). Retrocompat is load-bearing and proven: single-owner orgs sit within `includedSeats` → zero charge, zero behavior change, no single-user flow altered. The migration is idempotent + authored-only (deploy via CI→GHCR→Coolify).

**Milestone significance:** this turns the dormant `company_members` foundation (Phase 79) into REAL team seats — invite/roles/member-management plus per-seat billing whose price + included-seats are 100% super-admin-configurable (zero hardcoded billing numbers), riding the existing Stripe subscription, gated by `enforcementEnabled` (calibrate before charging), with every single-owner org byte-for-byte unchanged (retrocompat). The costly multi-tenancy work was already paid down (all RLS authorizes via `company_members`), so this was product + billing surface, not a data-model rebuild.

**Operational deferrals (carry-forward):** apply the `20260628000001` migration to remote via CI→GHCR→Coolify; set the real `seatPriceCents` + per-tier `includedSeats` in `billing_config` and flip `enforcementEnabled` on AFTER calibration (record-only until then); live UAT of an invite → email → accept (existing-user join + new-user signup-then-join) → role change / remove → seat-quantity sync against Stripe test mode. Deferred to v2 (SEATX-01..04): `viewer`/granular permissions, SSO/SCIM, per-action audit trail + seat analytics, ownership-transfer + multi-owner.
