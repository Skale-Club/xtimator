...
last_updated: "2026-06-24T09:07:18.000Z"
...
Next Up: **Phase 107 IN PROGRESS (1/3 plans — 107-01 provider seam + schema + injection hardening shipped)**. Next: `/gsd:execute-phase 107` for Plan 107-02 (OpenRouter-web adapter: separate `openrouter:web_search` call, engine exa default/native configurable + gated Anthropic-web `user_location` adapter) and Plan 107-03 (deterministic fixture adapter + golden fixtures + fixed clock for the eval gate). The 107-01 adapter stubs (`adapters/openrouter-web.ts`, `adapters/anthropic-web.ts`) are dormant no-op seams awaiting Plan 02.
