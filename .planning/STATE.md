...
Phase: 104 (Notification Channels & Preferences Revamp — NOTIF-01..07) — COMPLETE (4/4 plans). Wave 0 (104-00 scaffold) + Wave 1 (104-01 schema/categories/UI/migration) + Wave 2 (104-02 WhatsApp+SMS senders) + Wave 3 (104-03 super-admin WhatsApp-template panel) shipped. All 7 NOTIF requirements satisfied.
...
Plan: 103-03 complete (CI regression gate — EVAL-04). Phase 103 done; all 4 EVAL requirements satisfied.
Status: 103-03 shipped — the EVAL-04 CI regression gate lands; **Phase 103 COMPLETE, v4.5 milestone COMPLETE**. Added `.github/workflows/test.yml`: a secret-free GitHub Actions gate triggering on push to main+dev, pull_request, and workflow_dispatch (permissions: contents:read, runs-on ubuntu-latest, timeout 15m). Steps: actions/checkout@v4 → actions/setup-node@v4 (node 24, cache:npm) → `npm ci` → scoped `npx tsc --noEmit -p tsconfig.ci.json` → `npx vitest run tests/unit tests/eval` → the SAME suite run a SECOND time (the Wave-1 determinism invariant made permanent — a re-emergent isolation leak fails CI). Created `tsconfig.ci.json` (Option A / Pitfall 7): extends base tsconfig, narrows `include` to shipped runtime source (app/lib/components/hooks + instrumentation/sentry entrypoints), excludes tests/** → `npx tsc --noEmit -p tsconfig.ci.json` is GREEN where full `tsc` is red on the 9 pre-existing test-file-ONLY errors (0 in app/lib); `git diff tsconfig.json` empty (global config untouched — NOT Option B). SECRET-FREE: mocked providers (no AI keys) + tests/integration EXCLUDED by path-scoping to `tests/unit tests/eval` (integration needs live Supabase — Pitfall 6) → zero AI/Supabase keys; gitleaks clean on both commits. TEST gate ONLY: no `next build`, no docker/GHCR/Coolify, no deploy step; `build-deploy.yml` byte-untouched (deploy memory respected). Gate NOT hollow: `npx vitest run tests/eval` reports Test Files 2 (103-02 include fix in effect). LOCAL GATE SIMULATION GREEN: scoped tsc exit 0 + `npx vitest run tests/unit tests/eval` → 241 files passed | 3 skipped, 1712 passed | 31 todo. Added `.nvmrc` (node 24, local/CI parity) + `package.json` `test:eval` ("vitest run tests/eval"). 2 atomic commits: b0a6df1 (tsconfig.ci.json + .nvmrc + test:eval), 61b1cd3 (test.yml). EVAL-04 marked complete. The two pre-existing non-test working-tree files (hero-section.tsx, next-env.d.ts) + xphere untouched (never staged). DEFERRED non-blocking UAT (per 103-VALIDATION): open a throwaway PR breaking a metric and confirm the Actions check goes red (needs a live PR on the default branch — cannot run from a local executor). See 103-03-SUMMARY.md.
...
Last activity: 2026-06-23 - Completed quick task 260623-gbr: Fix TypeScript errors in test mocks blocking Docker build
