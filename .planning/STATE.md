...
Next Up: **Phase 107 COMPLETE (3/3 plans — 107-01 provider seam + schema + injection hardening, 107-02 OpenRouter-web + Anthropic-web real adapters, 107-03 deterministic fixture adapter + golden fixtures + fixed clock + gated eval source test)**. Next: `/gsd:verify-work 107`, then `/gsd:plan-phase 108` (orchestrator integration — the payoff: wire `getPriceResearchProvider()` + the Phase-106 cache into `generateEstimateForProject` after anchoring, evidence-gated tagging + no-$0 fallback ladder + metering + the "Couch cleaning 8seats" full-graph regression). All three adapters (`openrouter-web` SEPARATE `openrouter:web_search` engine exa/native, gated `anthropic-web` `user_location`, deterministic `fixture`) are evidence-gated and remain DORMANT — Phase 108 injects them. The `fixture` adapter is the eval-harness injection point (mirrors `tests/eval/mock-providers.ts`).
