...
Last activity: 2026-06-13 - Completed quick task 260613-coj: Routed the WhatsApp AI layer's three direct process.env.OPENAI_API_KEY reads through getIntegrationKey('openai') (agent.ts + intent-router.ts classifier & QUERY agent), closing the last ADMIN-06 violation (TEST-ENV-01). env-var-sweep passes; full WhatsApp unit suite green (189 passed); tsc clean on touched files; env-var-sweep test unmodified. Resolves the one genuine product gap from 260613-aoe — known-issues.md mock-drift residual now 9 tests/7 files, all FLAGGED test-rewrite/design-decision items (no product gaps)
