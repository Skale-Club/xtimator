...
last_updated: "2026-06-24T05:25:00.000Z"
...
Next Up: **Phase 107 IN PROGRESS (2/3 plans — 107-01 provider seam + schema + injection hardening, 107-02 OpenRouter-web + Anthropic-web real adapters shipped)**. Next: `/gsd:execute-phase 107` for Plan 107-03 (deterministic fixture adapter + golden `(service, region) → candidates` fixtures + fixed clock so the v4.5 eval harness + CI regression gate run green with zero live network). Both real adapters (`adapters/openrouter-web.ts` SEPARATE `openrouter:web_search` engine exa/native, `adapters/anthropic-web.ts` gated `user_location`) are evidence-gated + never-throw but remain DORMANT — Phase 108 wires `getPriceResearchProvider()` into the estimate pipeline.
