...
- 🚧 **v4.17 Admin Polish & Credit UX Compliance** — Phases 156-159 (roadmap created 2026-07-06) — fixes a real regression against a locked v4.15 decision (tenant-facing surfaces still leak raw credit numbers in 3 places on /settings/billing), then polishes the super-admin experience: nav reorg (Dashboard/Companies/Inbox first + new grouped "Content" section) + Legal Pages→Pages rename, two owner-flagged confusing labels fixed (Message→Message Template, Support Mode→View as Company), a credit-model-centric admin Billing page overhaul (replacing the stale tier/MRR view), and a "Premium Xtimator" glassmorphism visual redesign of the v4.16 Inbox. No new backend/ledger logic — 100% reuse of existing v4.7/v4.15 credit infrastructure.
...
## 🚧 v4.17 Admin Polish & Credit UX Compliance (Phases 156-159)
...
> **Numbering:** continues the GLOBAL phase counter. v4.16 ended at Phase 155. **v4.17 starts at Phase 156.** Do NOT reset to 1, and do NOT trust a stale `999.1`/`current_milestone` reading from `gsd-tools init` — ROADMAP.md's own phase history is the source of truth.
>
...
>
> **Locked scope guardrails:** No new backend/ledger logic — 100% reuse of the existing credit-ledger, `billing_config`, and cost-aggregation functions shipped in v4.7 (Phases 110-116) and v4.15 (Phases 150-153); this milestone is UI/UX polish + a compliance bugfix, not new billing logic. The credit-leak fix (Phase 156) repairs an already-violated locked decision (CREDITUI-04 from v4.15: tenants must NEVER see a raw credit count or $ figure) — ships first, independent of everything else. User-facing renames only — Legal Pages' public routes (`/privacy-policy`, `/terms-of-service`) and the `legal_pages` DB table are untouched (only the admin nav label + `/admin/legal` → `/admin/pages` route); Support Mode's internal functions (`startSupportSession`, `getSupportModeSession`, `endSupportSession`), audit-log action literals (`company.support_mode_start`/`company.support_mode_end`), cookie name (`support_mode_session`), and file names are unchanged — only user-facing copy changes to "View as Company". The "Content" sidebar group is NEW UI work — today's admin nav (`components/admin/admin-nav.tsx`) is a flat unordered list with zero grouping/section-header pattern; this milestone builds that pattern. The Inbox redesign reuses Xtimator's own Phase-71 glass tokens/components (`--glass-*` CSS vars, `Card variant="glass"`, `Avatar`/`AvatarFallback`) — not a literal Xphere clone; a deterministic name→color initials-avatar utility needs to be built (none exists). All 4 phases are file-independent of each other (billing components vs nav/settings components vs admin/billing page vs admin/inbox components) — no blocking dependency spine; the numbered order below is the recommended execution sequence (most urgent/lowest-risk first, most subjective/highest-effort last), not a hard requirement.

### Phases

- [x] **Phase 156: Tenant Credit UX Compliance Fix** — Eliminate 3 confirmed raw-credit-number leaks on `/settings/billing`: `TopUpPackCard`'s "≈ X credits" subtext, `AutoTopupDialog`'s pack-picker `SelectItem` showing credits, and `CreditHistoryList`'s per-row credit deltas (e.g. "+2,000") — only the existing % bar + qualitative low/critical states may communicate usage; add a real visual progress-bar element to the topbar `CreditChip` (reusing `components/ui/progress.tsx` with the same color-escalation as `UsageProgressBar`), replacing today's plain "X% used" text; reconcile `TierCardsGrid`'s hardcoded `TIERS` array against `billing_config` reality (source from config where a field exists, or verify + document as intentionally-static where it doesn't). Highest priority — repairs a violated v4.15 locked decision. No dependency — ships first, independent of the other 3 phases. (CREDITFIX-01, CREDITFIX-02, CREDITFIX-03) (completed 2026-07-06)
- [x] **Phase 157: Admin Nav Reorg & Naming Fixes** — Reorder the super-admin sidebar so Dashboard, Companies, Inbox are the first three items; build a NEW "Content" grouped-nav UI pattern (none exists today) containing Landing Page, Pages, Blog, SEO, Branding under a group header, distinct from ungrouped items; rename "Legal Pages" → "Pages" (nav label + slug `/admin/legal` → `/admin/pages`, every hardcoded reference — nav entry, `revalidatePath` calls, internal links — retargeted; public routes and the `legal_pages` DB table untouched); rename tenant Settings sidebar "Message" → "Message Template" (label + page `<h1>` + `metadata.title`; route `/settings/estimate-templates` unchanged); rename super-admin "Support Mode" → "View as Company" in every user-facing string (row action button, persistent banner, exit button, both error toasts; internal function/audit-log/cookie/file names unchanged). No dependency — independent of Phase 156, different files. (NAV-01, NAV-02, NAV-03, NAMING-01, NAMING-02) (completed 2026-07-06)
- [x] **Phase 158: Admin Billing Page Credit-Model Overhaul** — Replace the admin `/admin/billing` page's tier/MRR-first view with a credit-model-centric primary view per company (credit balance, real AI cost, effective markup) reusing the already-shipped `getCompanyCostOverview`/`aggregateAiCostByOperation` functions; keep force-tier and grant-credits available as secondary actions on the same page (not removed, just no longer the visual focus); replace the platform-wide summary card's hardcoded `proCount*29 + bizCount*99` MRR calculation with a real aggregated credit/cost summary via the no-arg `aggregateAiCostByOperation()` mode. No dependency — independent of Phases 156/157, different admin page/files. (BILLADMIN-01, BILLADMIN-02, BILLADMIN-03) (completed 2026-07-06)
- [x] **Phase 159: Inbox Visual Redesign — "Premium Xtimator"** — Apply the existing Phase-71 glassmorphism design system (`--glass-*` CSS vars, `Card variant="glass"`, `Avatar`/`AvatarFallback` primitives) to the v4.16 Inbox: deterministic-color initials avatars per contact (new utility — same contact always renders the same color), glass-surface list rows + thread pane (replacing the flat `bg-muted`/plain-border treatment), a visually rich unread state (colored accent bar/dot, not just a text `Badge`), and the same glass treatment applied to the Inbox Settings sub-page (`/admin/inbox/settings`, Accounts + Templates tables) for consistency. No dependency — independent of the other 3 phases, only touches `app/admin/inbox/*`. Sequenced last as the largest/most subjective single effort. (INBOX-05, INBOX-06, INBOX-07, INBOX-08) (completed 2026-07-06)

### Phase Details — v4.17 Admin Polish & Credit UX Compliance

### Phase 156: Tenant Credit UX Compliance Fix

**Goal**: Every tenant-facing surface fully complies with the v4.15 CREDITUI-04 locked decision (never show a raw credit count or $ figure) — closing the 3 confirmed leaks — and the topbar credit indicator gains a real visual progress bar instead of plain text; the Plans page's tier copy is verified accurate against `billing_config`.
...
**Success Criteria** (what must be TRUE):
  1. A grep/static check across `TopUpPackCard`, `AutoTopupDialog`, and `CreditHistoryList` confirms zero remaining raw-credit-number renders (no "≈ X credits", no per-row "+N"/"-N" credit deltas) — only qualitative/percentage-based copy remains
...
Plans:
- [x] 156-01-PLAN.md — Fix 3 raw-credit-number leaks (TopUpPackCard, AutoTopupDialog, CreditHistoryList) + real progress bar in CreditChip
...
**Success Criteria** (what must be TRUE):
  1. Loading the super-admin panel shows Dashboard, Companies, Inbox as sidebar items 1-3, in that order
  2. The sidebar shows a visually distinct "Content" group header containing Landing Page, Pages, Blog, SEO, Branding as its members, using a newly-built grouped-nav component (not just reordered flat items)
...
- [x] 157-03-PLAN.md — Update Support Mode test string assertions to match View as Company / Exit view copy
**UI hint**: yes
...
**Success Criteria** (what must be TRUE):
  1. Opening `/admin/billing` shows, as the primary/most prominent content per company, a real credit balance + real AI cost + effective markup (sourced from `getCompanyCostOverview`/`aggregateAiCostByOperation`) rather than tier/MRR figures
...
**Plans**: 1 plan
Plans:
- [x] 158-01-PLAN.md — Credit-model overhaul: platform summary card data-source swap + per-company table restructure (credit balance/real cost/auto-top-up primary, force-tier/grant-credits collapsed into Manage disclosure)
**UI hint**: yes

### Phase 159: Inbox Visual Redesign — "Premium Xtimator"

**Goal**: The v4.16 Inbox (conversation list + thread pane + Settings sub-page) visually matches the rest of the admin's Phase-71 glassmorphism design system instead of looking like a flat, avatar-less, plain-gray table — without changing any read-only behavior (no reply/send, no real-time updates).
...
**Success Criteria** (what must be TRUE):
  1. Each conversation-list row displays an initials avatar whose background color is deterministically derived from the contact's name/identifier — the same contact always renders the same color across reloads
  2. The conversation list rows and the thread pane both use the `--glass-*` CSS vars / `Card variant="glass"` treatment (verifiable by inspecting class names/component props) instead of `bg-muted`/plain borders
...
**Plans**: 2 plans
Plans:
- [x] 159-01-PLAN.md — Avatar utility + conversation-list row and thread-pane header glass redesign (INBOX-05, INBOX-06, INBOX-07)
- [x] 159-02-PLAN.md — Inbox Settings sub-page glass consistency verification + polish (INBOX-08)
**UI hint**: yes
