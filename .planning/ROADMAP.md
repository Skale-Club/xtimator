...
- 🚧 **v3.1.1 MVP Launch Prep + Future-Proofing** — Phases 66-69 (started 2026-05-15, rescoped same day for Inngest + Storage abstraction)
...
- [ ] **Phase 70: Stripe Connect — Optional Customer Payments on Estimates** — Ship an entirely-optional Stripe Connect Standard integration so service businesses can connect their existing Stripe account once (via OAuth in Settings → Payments) and instantly get a "Pay Now" button on every shared estimate. Customer clicks → Stripe Checkout (hosted by Stripe, on the business's connected account) → pays full amount → webhook marks `estimates.payment_status = 'paid'`, emails business owner, emails customer branded receipt, shows banner on share page after redirect. Zero application fee (Xtimator already monetizes via SaaS plans). Everything works perfectly without Stripe connected — no broken UI, no upsell nag, share/PDF/email flows unchanged. Harvests SEED-020. (CONNECT-01..09)
...
| 70. Stripe Connect — Customer Payments | v3.1.1 | 4/5 | In Progress|  |
