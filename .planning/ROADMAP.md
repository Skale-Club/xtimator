...
- âœ… **v1.0 MVP** â€” Phases 1-8 (shipped 2026-04-21) Â· [archive](milestones/v1.0-ROADMAP.md)
- âœ… **v1.1 Dark-first UX & Modern Redesign** â€” Phase 9 (shipped 2026-04-22) Â· [archive](milestones/v1.1-ROADMAP.md)
- âœ… **v1.2 Brand Identity & Global Reach** â€” Phases 10-18 (shipped 2026-05-06) Â· [archive](milestones/v1.2-ROADMAP.md)
- âœ… **v1.3 Smart Pricing** â€” Phases 19-23 (shipped 2026-05-08) Â· [archive](milestones/v1.3-ROADMAP.md)
- âœ… **v1.4 Estimate Plain Text & Pricing Tools** â€” Phases 24-26 (shipped 2026-05-08) Â· [archive](milestones/v1.4-ROADMAP.md)
- âœ… **v1.5 Zero-friction Project Onboarding** â€” Phases 27-30 (shipped 2026-05-09)
- âœ… **v1.6 Multi-modal Project Input** â€” Phases 31-33 (shipped 2026-05-09)
- âœ… **v1.7 Client-Project Quick Actions** â€” Phase 34 (shipped 2026-05-09)
- âœ… **v1.8 Iterative Estimate Refinement** â€” Phases 35-37 (shipped 2026-05-09)
- âœ… **v1.9 Custom Domain Support** â€” Phases 38-39 (shipped 2026-05-10)
- âœ… **v2.0 WhatsApp Estimate Channel** â€” Phases 40-45 (shipped 2026-05-10)
- âœ… **v2.1 WhatsApp Launch-Readiness** â€” Phases 46-52 (shipped 2026-05-11)
- âœ… **v2.2 WhatsApp Channel Polish** â€” Phases 53-54 (shipped 2026-05-13)
...
<details>
<summary>âœ… v1.1 Dark-first UX & Modern Redesign (Phase 9) â€” SHIPPED 2026-04-22</summary>
...
<details>
<summary>âœ… v1.2 Brand Identity & Global Reach (Phases 10-18) â€” SHIPPED 2026-05-06</summary>
...
<details>
<summary>âœ… v1.3 Smart Pricing (Phases 19-23) â€” SHIPPED 2026-05-08</summary>
...
<details>
<summary>âœ… v1.4 Estimate Plain Text & Pricing Tools (Phases 24-26) â€” SHIPPED 2026-05-08</summary>
...
- [x] **Phase 27: Capture Schema Migration** â€” Make `recordings.storage_path` nullable and `projects.client_id` optional so text-path and client-optional flows are unblocked (completed 2026-05-08)
...
- [x] **Phase 29: Frictionless Project Creation & Client Linking** â€” Remove mandatory client step from project wizard; add New Project button on client detail page; show Link Client card in project Overview when no client is set
 (completed 2026-05-09)
- [x] **Phase 30: AI Client Extraction** â€” After estimate generation, surface a non-blocking toast when AI detects a client name in content, letting the user accept or dismiss the suggested link
 (completed 2026-05-09)

### v1.6 Multi-modal Project Input (Phases 31-33)

- [x] **Phase 31: Wizard Modality Selection** â€” Add second step to project wizard with 3 modality choice cards (Audio/Text/Photos); redirect to appropriate route based on selection; store input_mode on project (1 plan) (completed 2026-05-09)
- [ ] **Phase 32: Text Input Route** â€” New `/projects/[id]/describe` route with large textarea; save text as transcript; "Generate Estimate" button triggers same pipeline as audio (1 plan)
  - [x] 32-01-PLAN.md â€” Route shell + TextDescribe component + generate pipeline wiring
- [ ] **Phase 33: Photos Input Route** â€” New `/projects/[id]/photos-input` route with direct upload; "Generate from Photos" button prominent when photos added; Claude Vision pipeline
  - [x] 33-01-PLAN.md â€” Route shell + PhotosInput component + generate pipeline wiring

### v1.9 Custom Domain Support (Phases 38-39)

- [x] **Phase 38: Custom Domain DB + Settings UI** â€” Add `custom_domain` column to companies table; add domain input field + DNS/CNAME setup instructions to settings page; companies without a domain configured are unaffected (completed 2026-05-10)
- [x] **Phase 39: Subdomain Routing + White-label Estimate View** â€” Detect custom host in `proxy.ts`; rewrite requests to `/estimate/{token}` without redirect; hide "Generated by Xtimator" footer when estimate is served from a custom domain (completed 2026-05-10)

### v2.0 WhatsApp Estimate Channel (Phases 40-45)

- [x] **Phase 40: Webhook Infrastructure** â€” DB tables (`company_whatsapp`, `whatsapp_sessions`, deduplication), `WhatsAppProvider` interface + `MetaAdapter` skeleton, `POST /api/webhooks/whatsapp` with HMAC-SHA256, `GET` hub.challenge verification, proxy.ts bypass, admin panel Meta token card (completed 2026-05-10)
- [ ] **Phase 41: Generate-Estimate Service Extraction** â€” Extract business logic from `app/api/generate-estimate/route.ts` into `lib/services/generate-estimate.ts` callable with `(companyId, projectId)` â€” no auth context required; API route becomes a thin wrapper; enables webhook handler to invoke the pipeline directly
- [ ] **Phase 42: Inbound Processing** â€” `lib/whatsapp/handler.ts` state machine (awaiting_input state); audio messages â†’ Whisper â†’ estimate; text messages â†’ transcript â†’ estimate; photo messages â†’ Claude Vision â†’ estimate; sends confirmation summary to owner; session created and transitioned to awaiting_confirm
- [ ] **Phase 43: Confirmation Flow** â€” `awaiting_confirm` state machine â€” "send" / "cancel" command parsing; session expiry at 30 minutes with expiry notification; `pg_cron` or Vercel cron cleanup; `lib/whatsapp/formatter.ts` confirmation message builder
- [ ] **Phase 44: Outbound Client Delivery** â€” Deliver estimate to client as share link (default) or formatted text per `company_whatsapp.delivery_format`; update estimate + project status to "sent"; confirm delivery to owner via WhatsApp
- [ ] **Phase 45: Settings UI + Admin Token** â€” `/settings/integrations` page with WhatsApp Connect Card (connect / verify OTP / disconnect / delivery format selector); Settings entry card; admin panel Meta access token card; `POST /api/settings/whatsapp` connect/verify/delete routes

### v2.1 WhatsApp Launch-Readiness (Phases 46-52) â€” âœ… SHIPPED 2026-05-11

- [x] **Phase 46: Typed Error Handling Foundation** â€” `lib/errors/` with `XtimatorError` class, type+surface composite codes, `asResponse()` wrapper, WhatsApp adapter (`handleWhatsAppError`), `throwIf*` helpers; foundation for all other v2.1 phases (SEED-014 harvested)
- [x] **Phase 47: Redis + Rate Limiting Infrastructure** â€” Upstash Redis client in `lib/redis.ts`; `lib/ratelimit.ts` with `rateLimit(limitName, identifier)`; applied to generate-estimate, webhooks/whatsapp, analyze-photos, translate (SEED-012 harvested)
- [x] **Phase 48: WhatsApp Multi-Message Debounce** â€” Redis-backed buffer per phone_number; PUSH on inbound, 5s silence wait, GET+process all together; new `processInboundMessages()` accepts array; generate ONE estimate from aggregated input (SEED-010 harvested)
- [x] **Phase 49: WhatsApp Typing + Read Receipts** â€” `markMessageAsRead()` + `sendTypingIndicator()` in `lib/whatsapp/client.ts`; called after dedup pass and before heavy processing; re-send typing before 25s timeout (SEED-011 harvested)
- [x] **Phase 50: WhatsApp OTP Number Verification** â€” Two-step setup flow: submit credentials â†’ status=pending â†’ 6-digit code via WhatsApp â†’ verify â†’ status=active; schema columns (verification_code, attempts, expires); UI second step in `WhatsAppConnectCard` (SEED-015 Gap 2 harvested)
- [x] **Phase 51: WhatsApp Pre-Send Edit Commands** â€” Structured parser for `edit total/timeline/payment/summary`, `client`, `regenerate` commands; mutations on estimate; re-send updated summary; session stays in awaiting_confirm. Section/item-level edits deferred (SEED-015 Gap 1 partial)
- [x] **Phase 52: Per-Estimate Language Selection** â€” `language` column on estimates (default 'en'), `preferred_language` on clients, `default_estimate_language` on companies; cascade resolver; AI prompt parameterized; WhatsApp formatter localized; auto-learn client preference after send (SEED-016 backend harvested)

### v2.2 WhatsApp Channel Polish (Phases 53-54)

- [x] **Phase 53: PDF Attachment Delivery** â€” Add `pdf_attachment` as a third `delivery_format` option; generate PDF via existing `/api/estimates/[id]/pdf` endpoint; upload to `estimates-pdf` Supabase Storage with 24h signed URL; send to client via Meta API `type: "document"`; degrade to `share_link` on any failure (SEED-015 Gap 3) (completed 2026-05-11)
- [x] **Phase 54: WhatsApp Status Flow** â€” Wire full `pending â†’ verified â†’ active â†’ suspended` pipeline post-OTP; clear UI labels in `WhatsAppConnectCard`; admin/owner suspend and reactivate action; `handler.ts` gate enforces `status = 'active'` (SEED-015 Gap 5) (completed 2026-05-13)

<details>
<summary>âœ… v3.0 Monetization (Phases 55-60) â€” SHIPPED 2026-05-14</summary>
...
<details>
<summary>âœ… v3.1 Production Go-Live (rescoped) â€” Phase 61 only Â· SHIPPED 2026-05-15</summary>

| Phase | Name | Plans | Status |
|-------|------|-------|--------|
| 61 | Production Database Foundation | 5/5 | Complete 2026-05-15 |
| 62 | ~~Vercel Deployment + Custom Domain~~ | â€” | REMOVED â€” hosting migration tracked in SEED-018 |
| 63 | Stripe Live Mode Activation | â€” | DEFERRED â†’ v3.2 |
| 64 | Monitoring + Backup & Resilience | â€” | DEFERRED â†’ v3.2 |
| 65 | Production UAT + Bug Triage | â€” | DEFERRED â†’ v3.2 |
...
**Success Criteria** (what must be TRUE):
  ...
  2. Clicking 'Show me around' launches a 5-step spotlight overlay that highlights: New Project button, Projects link, Clients link, Price Book link, Language toggle â€” all without requiring page navigation
  ...
  5. A '?' floating button (fixed bottom-right) reopens the welcome modal in review mode ('Show me around' + 'Close' â€” no 'Start estimating') at any time; button is hidden during spotlight
**Plans**: 4 plans in 
Plans:
- [x] 74-01-PLAN.md â€” Tour infrastructure (TourProvider, use-tour hook, welcome modal, onboarding cookie wiring)
- [x] 74-02-PLAN.md â€” Spotlight walkthrough (5-step overlay, TourSpotlight, data-tour attributes on 5 targets)
- [x] 74-03-PLAN.md â€” Contextual first-visit tooltips (ContextualTooltip component, 5 tooltip placements)
- [x] 74-04-PLAN.md â€” '?' floating button + WelcomeModal review mode + final wiring + TypeScript clean pass

### Phase 71: Glassmorphism Structural Redesign â€” All Surfaces
**Goal**: Every surface a paying Xtimator customer touches feels premium and modern â€” frosted-glass cards, vibrant brand-tinted gradients on hero zones, stronger typography hierarchy, generous whitespace â€” without changing information architecture, navigation, or copy. Information stays where it is; presentation gets a Stripe-Dashboard-tier overhaul.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Every shadcn primitive in `components/ui/*` gains optional glass/gradient variants without breaking existing call sites â€” `<Card variant="glass">`, `<Button variant="primary">` with gradient bg + shimmer hover, `<Dialog>` with backdrop-blur, `<Input>` with gradient focus border, `<Badge>` with gradient status variants, `<Tabs>` with gradient indicator
  ...
  6. Customer-facing share page (`/estimate/[token]`) and "Pay Now" surfaces from Phase 70 use the new glass + gradient styling â€” Pay Now button gets brand gradient + subtle shimmer; success banner gets glass + success gradient
  ...
  8. Brand identity unchanged â€” `#406EF1` is still the primary, logo + wordmark are byte-identical, dark mode is still default, scoped themes (`[data-theme="dark-auth"]`, forced-light `/estimate/*`) still work
  9. Playwright visual snapshot baselines updated for every redesigned surface (expected â€” every existing snapshot WILL break and must be re-minted; no false-positive regressions allowed in the test suite after wave 5 lands)
  10. Performance: Lighthouse Performance + Accessibility scores stay â‰¥ 80 on `/` and `/dashboard` after redesign; First Load JS for `/dashboard` stays under 500 KB; `backdrop-filter: blur()` restricted to top surfaces only (hero, modals, sidebar â€” NOT every list row) so mid-range mobile GPUs stay smooth; `prefers-reduced-transparency` honored with solid-bg fallback
**Plans**: TBD (estimated 10 plans across 5 waves â€” see SEED-022 for wave breakdown)
**UI hint**: yes (this entire phase is UI â€” run `/gsd:ui-phase 71` to generate UI-SPEC.md BEFORE planning)

### Phase 72: Admin Menu Performance â€” Instant Navigation
**Goal**: Eliminate the perceived lag when opening admin menus in both the super-admin panel (`/admin/*`) and the client-facing app shell. Menus must open and render a skeleton within 100ms of click; no blank flash or layout shift. Root causes identified: (1) layout-level `Promise.all()` with no Suspense boundaries blocks the entire layout render including nav; (2) admin pages use `force-dynamic` with no caching, regenerating expensive queries on every navigation; (3) integrations page has an N+1 decrypt + `getUserById()` pattern; (4) admins page fetches 1000 users on load; (5) no loading skeletons â€” blank screen feels broken.
...
**Success Criteria** (what must be TRUE):
  1. `app/admin/layout.tsx` and `app/(app)/layout.tsx` wrap slow data-fetching sections in `<Suspense>` with skeleton fallbacks â€” nav and topbar render immediately while page content loads behind a skeleton
  ...
  3. `app/admin/integrations/page.tsx` N+1 decrypt + `getUserById()` loop replaced with a single JOIN query + batch decrypt â€” page load time drops from O(n) round-trips to O(1)
  4. `app/admin/admins/page.tsx` drops the `listUsers({ perPage: 1000 })` call; replaced with a paginated fetch (first 50) + server-side search, or a cached count query â€” no unbounded user list load on every nav
  5. Every admin page (`/admin/*`) and every app-shell nav transition shows a skeleton within 100ms â€” measured by adding `loading.tsx` to every admin route segment that lacks one
...
Plans:
- [x] 72-01-PLAN.md â€” 10 loading.tsx skeleton files for all admin routes + revalidate=60 ISR on 4 stable admin pages (PERF-ADMIN-01, PERF-ADMIN-02)
- [x] 72-02-PLAN.md â€” Admin layout Suspense boundary + app shell getBranding parallelized with getCachedCompany (PERF-ADMIN-03, PERF-ADMIN-06)
- [x] 72-03-PLAN.md â€” integrations N+1 getUserById batch fix + admins page listUsers(1000) replaced with bounded getUserById per row (PERF-ADMIN-04, PERF-ADMIN-05, PERF-ADMIN-06)

### Phase 66: Storage Abstraction Layer
**Goal**: Every storage call site in the app routes through a `lib/storage/` provider interface so swapping Supabase Storage for an S3-compatible backend (Hetzner Object Storage, MinIO, etc.) is a 1-line provider change. Default provider stays Supabase; the S3 path ships as a working skeleton validated against MinIO. Sequenced first because doing the refactor under live customer load (post-launch) is much riskier than now during a clean window â€” and Phase 67 (Inngest) workers can use the new `storage.*` API from day one with no follow-up refactor.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Running `grep -r "supabase.storage.from" app/ lib/ components/` returns zero hits outside `lib/storage/` â€” every audio, photo, PDF, logo, branding asset, and WhatsApp media call site uses the new `storage.*` API
...
Plans:
- [x] 66-01-PLAN.md â€” Wave 0 RED contract tests + StorageProvider interface + Supabase provider + buildStorageKey helper (STORAGE-01, STORAGE-02, STORAGE-04)
- [x] 66-02-PLAN.md â€” Migrate all 8 production call sites (logos â†’ audio â†’ photos â†’ pdfs â†’ wa-media) + 3 affected tests (STORAGE-03, STORAGE-04)
- [x] 66-03-PLAN.md â€” S3 provider skeleton + STORAGE_PROVIDER env gate + MinIO smoke script + docs/STORAGE-MIGRATION.md runbook (STORAGE-05, STORAGE-06, STORAGE-07)

### Phase 67: Inngest Background AI Job Processing
**Goal**: All long-running AI calls (estimate generation, audio transcription, photo analysis) run as Inngest background jobs so the API routes return in under 1 second â€” unblocking the Vercel Free 10-second function timeout while keeping the same UX (live "Saving / Transcribing / Analyzing / Generating" progress) via job-status polling. Inngest is forward-compatible with the future Hetzner host (no swap needed at migration time).
**Depends on**: Phase 66 (Inngest worker functions consume the new `storage.*` API from day one â€” no follow-up refactor required)
**Requirements**: INNGEST-01, INNGEST-02, INNGEST-03, INNGEST-04, INNGEST-05, INNGEST-06, INNGEST-07, INNGEST-08
**Success Criteria** (what must be TRUE):
  1. `POST /api/generate-estimate`, `POST /api/transcribe`, and `POST /api/analyze-photos` each return `{ jobId }` in under 1 second â€” the actual Anthropic / Whisper / Vision call runs inside an Inngest worker function and `usage_events` is recorded only on job success
  2. The capture screen displays a live "Saving / Transcribing / Analyzing / Generating" stepper driven by `GET /api/jobs/[jobId]` polling, and the user lands in the estimate editor when the final job completes â€” same UX as today, no perceivable regression
  ...
  4. The WhatsApp inbound handler dispatches Whisper / Vision work via Inngest instead of awaiting it inline â€” the webhook ack returns to Meta in well under 10 seconds even on long voice notes
...
Plans:
- [x] 66-01-PLAN.md â€” Wave 0 RED test stubs + verify usage_events idempotency UNIQUE index
- [x] 66-02-PLAN.md â€” Inngest install + client + serve handler + 4 worker functions (generateEstimateJob, transcribeAudioJob, analyzePhotosJob, whatsAppProcessJob)
- [ ] 66-03-PLAN.md â€” Refactor 3 AI routes to dispatch + create new /api/transcribe + /api/jobs/[jobId] proxy
- [ ] 66-04-PLAN.md â€” Refactor lib/whatsapp/handler.ts:processInboundMessages to dispatch via Inngest
- [ ] 66-05-PLAN.md â€” useJobStatus hook + capture-recorder.tsx polling + docs/INNGEST-LOCAL-DEV.md
**UI hint**: yes

### Phase 68: Hetzner Cloud Deploy-Readiness Artifacts
**Goal**: The repository ships every artifact required to deploy to a Hetzner Cloud VPS (Dockerfile, docker-compose with Caddy, `/api/health`, runbook) so the future v3.2 deploy is mechanical â€” but nothing is activated in this milestone. Local Docker build is proven to boot the app correctly. Sequenced after Storage + Inngest so the `/api/health` storage check uses the new `storage.*` API and the Docker image bundles the Inngest worker route.
...
**Success Criteria** (what must be TRUE):
  ...
  2. A developer can run `docker build -t xtimator . && docker run -p 3000:3000 --env-file .env.local xtimator` on Windows / macOS / Linux â€” the app boots, `/api/health` returns 200 with `{ ok, db, storage, commit }`, and a fresh signup + login flow completes against dev Supabase
...
**Success Criteria** (what must be TRUE):
  1. Every v2.2 + v3.0 + Inngest + Storage UAT test (`UAT-V22-*`, `UAT-V30-*`, `UAT-INNGEST-*`, `UAT-STORAGE-01`, `UAT-E2E-*`) has a pass-or-fail verdict line in `.planning/known-issues.md` â€” no silent successes
  ...
  3. The 8-minute long-audio test completes successfully via Inngest â€” proving the timeout-killer that would have failed on Vercel Free without the background-job refactor; tier enforcement (free 402, pro/business higher caps), Stripe test-mode happy path, Customer Portal, trial automation (T-3/T-0 emails arrive), admin force-tier + bonus credits + MRR view all observable
  4. Every storage path validated post-refactor â€” audio upload, photo upload, PDF generation, logo upload, and WhatsApp inbound media â€” uses the new `storage.*` API and works against Supabase
...
### Phase 70: Stripe Connect â€” Optional Customer Payments on Estimates
**Goal**: Any service business that connects their Stripe account once via OAuth in Settings â†’ Payments gets a "Pay Now" button on every shared estimate. Customer pays the full estimate total via Stripe Checkout hosted on the business's connected account; webhook marks the estimate paid, emails business owner, emails customer branded receipt, shows banner on share page. The integration is 100% optional â€” companies without Stripe connected see zero Stripe UI anywhere and all existing share/PDF/email flows work unchanged. Application fee is 0% (provider keeps 100%; Xtimator monetizes via SaaS plans only). Harvests SEED-020.
...
**Success Criteria** (what must be TRUE):
  1. A business owner can navigate to Settings â†’ Payments, click "Connect Stripe Account", complete Stripe OAuth (test mode in dev), and return to a page showing "Connected âœ“ as [account display name]" with a Disconnect button â€” `companies.stripe_account_id` is populated and `stripe_connect_status = 'active'`
  ...
  3. On the same shared estimate page, if the company has no `stripe_account_id` OR `payment_status == 'paid'`, no Pay Now button is shown and the existing share UI (accept/decline, PDF download, branded view) renders exactly as before â€” confirmed via two snapshot tests, one with Stripe connected and one without
  ...
  5. After payment, the business owner receives a Resend email "You received $X from [customer]" with a link to the estimate, and the customer receives a Resend branded receipt email "Payment confirmation â€” $X paid to [business]"; the customer is redirected back to `/estimate/[token]?stripe=success` where a green "âœ“ Payment received â€” thank you!" banner is shown and the Pay Now button is gone
  6. A business owner can click Disconnect in Settings â†’ Payments and `stripe_account_id` is cleared, `stripe_connect_status = 'disconnected'` â€” existing paid estimates retain their `paid` status; new shared estimates simply lose the Pay Now button (graceful degrade, no errors)
  7. The platform owner can add a `stripe_connect_client_id` (`ca_...`) value via `/admin/integrations` (managed via existing `platform_integrations` table pattern); when this is unset, Settings â†’ Payments shows "Stripe Connect not yet enabled on the platform â€” contact support" and never attempts an OAuth redirect that would 404
**Plans**: TBD
...
**Plans**: 3 plans
Plans:
- [x] 24-01-PLAN.md â€” Migration + pure utility (resolveTemplate, TEMPLATE_DEFAULTS, zod schema, CompanySettings extension, query function) with TDD
- [x] 24-02-PLAN.md â€” Server action (saveEstimateTemplate) + client form component (EstimateTemplateForm)
- [x] 24-03-PLAN.md â€” Settings sub-route page + loading skeleton + /settings entry card
**UI hint**: yes
...
**Plans**: 2 plans
Plans:
- [x] 25-01-PLAN.md â€” buildItemsBreakdown utility function + unit tests (TDD REDâ†’GREEN)
- [x] 25-02-PLAN.md â€” PlainTextCard component + data chain wiring (page.tsx â†’ ProjectWorkspace â†’ SendTab â†’ PlainTextCard)
**UI hint**: yes
...
**Depends on**: Phase 20 (Price Book CRUD UI â€” needs existing items to adjust), Phase 19 (price_source column in place)
...
**Success Criteria** (what must be TRUE):
  ...
  3. After confirming, all item prices in that category update simultaneously â€” no partial saves leave some items at old prices
**Plans**: 2 plans
Plans:
- [x] 26-01-PLAN.md â€” bulkAdjustSchema + bulkAdjustPriceBookCategory server action (test-first: Wave 0 RED stubs + Wave 1 implementation)
- [x] 26-02-PLAN.md â€” BulkAdjustDialog component + PriceBookList wiring (Adjust % button + live preview table)
...
**Requirements**: (infrastructure prerequisite â€” unblocks CAPTURE-02, CAPTURE-04, CLIENTASSOC-01, CLIENTASSOC-04)
**Success Criteria** (what must be TRUE):
...
**Plans**: 1 plan
Plans:
- [x] 27-01-PLAN.md â€” DB migration (nullable storage_path) + TypeScript type propagation + optional clientId schema + caller null-guards

### Phase 28: Unified Capture Screen
**Goal**: Users can provide audio, typed description, or photos as co-equal inputs on the capture screen â€” alone or combined â€” and generate an estimate from any combination
...
**Success Criteria** (what must be TRUE):
  ...
  2. A user who types a job description and taps Generate Estimate â€” without recording any audio â€” gets a generated estimate using that text as the input
  3. A user who uploads one or more photos â€” without recording audio or typing text â€” can tap Generate Estimate and receive an estimate derived from those photos
...
**Success Criteria** (what must be TRUE):
  1. A user can complete the new project wizard and reach the capture screen without selecting or creating a client â€” the client field is optional, not blocking
...
**Success Criteria** (what must be TRUE):
  1. When the AI detects a client name in the transcript, description, or photo analysis, a toast notification appears after estimate generation with the detected name â€” it does not interrupt or block the estimate editor
...
**Success Criteria** (what must be TRUE):
  1. After selecting a client (or skipping), the user sees 3 large clickable cards labeled "Audio", "Text", and "Photos" â€” each with an icon and a one-line use case description
...
**Plans**: 1 plan
Plans:
- [x] 31-01-PLAN.md â€” Database migration + types + schema + StepModalitySelect component + 2-step wizard + action updates
**UI hint**: yes
...
**Success Criteria** (what must be TRUE):
  ...
  4. The estimate generation pipeline runs identically to the audio route â€” the only difference is the text origin
...
- [x] **Phase 34: Client-Project Quick Actions Verification** â€” Verify all CLIENTASSOC features work correctly; address any gaps (completed 2026-05-09)
   - [x] 34-01-PLAN.md â€” Verification plan (4 human checkpoint tasks)

### v1.8 Iterative Estimate Refinement (Phases 35-37)

- [x] **Phase 35: Text Refinement** â€” Add text input refinement panel to estimate editor, new API endpoint `/api/estimates/[id]/refine`, AI refinement prompt, new version creation
   - [x] 35-01-PLAN.md â€” Text refinement panel + API endpoint + AI integration (COMPLETE)
- [x] **Phase 36: Voice Refinement** â€” Inline voice recorder (~30s), Whisper transcription, same refinement pipeline
   - [x] 36-01-PLAN.md â€” VoiceRefineRecorder + voice API route + panel wiring (COMPLETE 2026-05-09)
- [x] **Phase 37: Photo Refinement** â€” Photo upload, Claude Vision analysis, auto-generate instruction
   - [x] 37-01-PLAN.md â€” Photo upload section in refine panel + Claude Vision API route

### Phase 38: Custom Domain DB + Settings UI
**Goal**: Company owners can enter and save a custom domain from settings, and see DNS/CNAME instructions â€” companies without a domain configured are completely unaffected
...
**Success Criteria** (what must be TRUE):
  ...
  3. A company that leaves the custom domain field empty continues to generate and share estimates on `xtimator.com/estimate/{token}` â€” no change to any existing behavior
  4. The saved domain persists across sessions and is scoped to the company (not shared across companies)
**Plans**: 2 plans

Plans:
- [x] 40-01-PLAN.md â€” DB migration (3 WA tables + RLS + purge) + lib/whatsapp/ modules (types, verify, client) + unit tests
- [x] 40-02-PLAN.md â€” Webhook route (GET challenge + POST HMAC handler + dedup stub) + proxy.ts bypass
**UI hint**: yes

### Phase 39: Subdomain Routing + White-label Estimate View
**Goal**: Requests arriving at a company's custom domain render the correct estimate directly â€” no redirect, no Xtimator branding in the footer
**Depends on**: Phase 38 (custom_domain column must exist and be populated)
**Requirements**: DOMAIN-03, DOMAIN-04
**Success Criteria** (what must be TRUE):
  1. A browser request to `estimates.mycompany.com/estimate/{token}` renders the estimate page with the company's logo, name, and colors â€” without redirecting to xtimator.com
  ...
  3. The same share token accessed via `xtimator.com/estimate/{token}` continues to render normally with the Xtimator footer intact â€” no regression for standard links
**Plans**: 1 plan
Plans:
- [x] 39-01-PLAN.md â€” proxy.ts custom host detection + EstimateView white-label prop + estimate page header wiring + unit tests
**UI hint**: yes

### Phase 40: Webhook Infrastructure
**Goal**: The system can receive, verify, and route inbound WhatsApp messages â€” the security and data foundation for every subsequent phase
...
**Success Criteria** (what must be TRUE):
  ...
  3. A second inbound POST carrying the same `wamid.*` message ID as a previously processed message is silently discarded â€” no duplicate estimate is created
...
**Requirements**: (internal prerequisite â€” enables WA-07, WA-08, WA-09)
**Success Criteria** (what must be TRUE):
  ...
  2. The existing `POST /api/generate-estimate` route calls this service function after its auth check â€” behavior from the authenticated UI path is unchanged
  3. Unit tests exercise the service function directly without an HTTP request or auth context
**Plans**: 2 plans

Plans:
- [ ] 40-01-PLAN.md â€” DB migration (3 WA tables + RLS + purge) + lib/whatsapp/ modules (types, verify, client) + unit tests
- [ ] 40-02-PLAN.md â€” Webhook route (GET challenge + POST HMAC handler + dedup stub) + proxy.ts bypass

### Phase 42: Inbound Processing
**Goal**: An owner can send audio, text, or a photo to the registered WhatsApp number and receive an estimate confirmation summary in reply â€” without opening the app
...
**Success Criteria** (what must be TRUE):
  1. Owner sends a voice note via WhatsApp â€” the bot transcribes it via Whisper, generates an estimate, and replies with a formatted summary of sections and total within the Meta 20-second response window (fire-and-forget async)
  2. Owner sends a text message describing a job â€” the bot generates an estimate from that text and replies with a confirmation summary
  3. Owner sends a photo of a job site â€” the bot analyzes it via Claude Vision, generates an estimate, and replies with a confirmation summary
...
**Success Criteria** (what must be TRUE):
  1. Owner replies "send" in the confirmation window â€” the session transitions to delivery and the owner receives a confirmation that the estimate is being sent to the client
  2. Owner replies "cancel" â€” the draft project and estimate are discarded and the bot confirms cancellation; no orphan records remain
  3. A session with no owner response expires after 30 minutes â€” the bot sends an expiry notification and the session is cleaned up from the database
**Plans**: 2 plans
...
**Success Criteria** (what must be TRUE):
  1. When delivery format is "share link" (default), the client receives a WhatsApp message containing the public estimate URL (`xtimator.com/estimate/{token}`) â€” no template approval required
...
**Success Criteria** (what must be TRUE):
  1. Owner navigates to `/settings/integrations`, sees a WhatsApp card, enters their Business phone number, receives a verification code, and confirms â€” the number shows as "Active" with a green indicator
  2. Owner clicks "Disconnect" and confirms the AlertDialog â€” the number is removed and future messages from that number are silently ignored
...
**Success Criteria**:
  1. User sends 5 messages in quick succession â€” system waits 5s after the last, then processes ALL together
...
**Success Criteria**:
  1. User submits credentials â†’ status='pending' â†’ 6-digit code sent via WhatsApp
  2. User enters code â†’ server validates (10min TTL, max 3 attempts) â†’ status='active'
...
**Goal**: Clients can receive their estimate as a PDF document via WhatsApp â€” a third delivery option alongside share link and formatted text
...
**Success Criteria** (what must be TRUE):
  1. Owner can select "PDF attachment" as the delivery format in WhatsApp settings â€” the option appears alongside "Share link" and "Formatted text" in the selector
  ...
  3. The PDF is the same branded document generated by the existing `/api/estimates/[id]/pdf` endpoint â€” line items, totals, company logo, and colors are correct
  4. If PDF generation, Supabase Storage upload, or the Meta document API call fails for any reason, the send completes anyway using the share_link fallback â€” the owner is not left with a failed delivery
**Plans**: 2 plans
Plans:
- [x] 53-01-PLAN.md â€” Wave 0 test stubs + DB migration (extend delivery_format CHECK) + lib/whatsapp/pdf-delivery.ts helper (generateAndUploadEstimatePDF + buildPdfFilename)
- [x] 53-02-PLAN.md â€” confirm.ts handleSend pdf_attachment branch + WhatsAppConnectCard third SelectItem

### Phase 54: WhatsApp Status Flow
**Goal**: The WhatsApp connection status pipeline is fully wired â€” UI shows accurate labels, transitions follow the correct sequence, admins can suspend and reactivate, and the message handler enforces the active gate
**Depends on**: Phase 50 (OTP verification established pendingâ†’active path), Phase 45 (WhatsAppConnectCard UI)
**Requirements**: WASTATUS-01, WASTATUS-02, WASTATUS-03, WASTATUS-04
**Success Criteria** (what must be TRUE):
  1. The WhatsApp Connect Card displays one of four human-readable labels â€” "Pending", "Verified", "Active", or "Suspended" â€” matching the current database status value; no raw enum values are shown to the user
  ...
  3. An owner (or admin) can set the connection status to `suspended` from the settings UI, and a suspended connection can be reactivated back to `active` â€” both actions persist correctly and are reflected immediately in the UI
  4. An inbound WhatsApp message from a number whose `company_whatsapp.status` is `pending`, `verified`, or `suspended` is silently ignored by the handler â€” no estimate is created, no reply is sent
**Plans**: 2 plans
Plans:
- [x] 54-01-PLAN.md â€” updateWhatsAppStatus server action + unit tests (WASTATUS-02, WASTATUS-03, WASTATUS-04)
- [x] 54-02-PLAN.md â€” WhatsAppConnectCard: StatusBadge + Suspend/Reactivate buttons (WASTATUS-01, WASTATUS-03)
...
| 62. ~~Vercel Deployment + Custom Domain~~ | v3.1 | â€” | REMOVED â€” see SEED-018 for Hetzner migration | - |
| 63. Stripe Live Mode Activation | v3.1 | 0/TBD | DEFERRED â†’ v3.2 | - |
| 64. Monitoring + Backup & Resilience | v3.1 | 0/TBD | DEFERRED â†’ v3.2 | - |
| 65. Production UAT + Bug Triage | v3.1 | 0/TBD | DEFERRED â†’ v3.2 | - |
...
| 70. Stripe Connect â€” Customer Payments | v3.1.1 | 5/5 | Complete    | 2026-05-17 |
...
**Success Criteria** (what must be TRUE):
  ...
  2. Every tooltip and spotlight step appears ONLY when its documented trigger fires â€” no unprompted popups on page load, refresh, or unrelated nav
  ...
  4. Dismissal persists per user â€” once dismissed, a tooltip never reappears unless the user explicitly opens "Restart tour" via `TourHelpButton`
  ...
  7. Manual UAT pass: open every page that hosts a tooltip in EN, PT, ES (3 locales Ã— N pages); confirm strings translated, position correct, animation gated, no overlap with sticky topbar or hero gradient

**Plans:** 4/4 plans complete

### Phase 76: Price Book CSV Pro â€” Professional Import UX
...
**Depends on:** Phase 19 (Price Book DB) Â· Phase 20 (Price Book CRUD UI) Â· Phase 21 (Initial CSV Import â€” the baseline this phase upgrades)

**Requirements:** PB-CSV-01, PB-CSV-02, PB-CSV-03, PB-CSV-04, PB-CSV-05, PB-CSV-06, PB-CSV-07, PB-CSV-08, PB-CSV-09, PB-CSV-10

**Success Criteria** (what must be TRUE):
  1. **4-step wizard** replaces single dialog â€” (1) Upload + drag-drop, (2) Column mapping (auto-detect with override), (3) Preview + per-row edit + dedupe strategy choice, (4) Import + verification summary. Progress indicator shows current step.
  2. **Column auto-detection** â€” recognizes common alias headers ("item", "service", "description" â†’ name; "price", "cost", "rate" â†’ unit_price; "category", "group", "folder" â†’ folder; "qty unit", "uom", "measure" â†’ unit). User can override every mapping via dropdown. Supports skipping unmapped columns.
  3. **Per-row inline editing** â€” every preview row's name/unit/unit_price/folder is editable directly in the table before commit. Validation errors (negative price, missing name, malformed currency) shown inline; row blocked from import only if user doesn't fix.
  4. **Locale-aware currency parsing** â€” accepts `$1,234.56` Â· `1.234,56` Â· `R$ 1.234,56` Â· `1234` Â· `1234.5` Â· `"$1,234.56"`. Owner sees detected locale guess + can override (US / BR / Custom decimal+thousands).
  5. **Duplicate resolution strategy** â€” when name+folder collides with existing row, user picks: Skip duplicates Â· Update existing (overwrite unit_price + unit + notes) Â· Import as new (suffix " (2)"). Default: Skip. Per-row override available.
  6. **Dry-run summary BEFORE commit** â€” shows: N rows will be inserted Â· N updated Â· N skipped Â· N folders created. User confirms before any DB write.
  7. **Undo last import** â€” every batch import writes a `price_book_imports` row capturing the inserted IDs; "Undo last import" button on price-book page deletes those rows (5-min window). Persisted in DB so survives reload.
  8. **Streaming progress for large files** â€” files >200 rows show real-time progress (e.g. "Importing 347 of 800â€¦") via React state ticks during the bulk insert. UI doesn't lock; cancel button available.
  9. **Error report download** â€” if any rows fail validation, user can download `import-errors.csv` with the failed rows + reason column. Same format as input so they can fix and re-upload.
  10. **Tests cover the new pipeline** â€” unit tests for: auto-detect aliases (8 cases), locale parsing (12 cases), dedupe strategies (3 cases Ã— scenarios), wizard state machine (step nav + persistence on close-reopen). Playwright E2E spec walks the full happy path with a 50-row fixture file.
...
### Phase 76.1: PWA â€” Progressive Web App Infrastructure (INSERTED)
...
**Depends on:** Phase 76 (Price Book CSV Pro â€” ensures app shell is stable before service worker layer is added)

**Requirements:** PWA-01, PWA-02, PWA-03, PWA-04, PWA-05, PWA-06

**Success Criteria** (what must be TRUE):
  1. **Installable** â€” Lighthouse PWA audit passes on mobile simulation (all installability checks green). App installs correctly on iOS Safari (Add to Home Screen) and Android Chrome (install prompt).
  2. **App shell offline** â€” Killing the network and reloading shows the app shell (not a browser error page). A branded offline fallback page renders at `/offline`.
  3. **Read-only cache** â€” Previously visited `/estimates` list and individual `/estimates/[id]` pages load from cache when offline. Stale-while-revalidate for Supabase GET calls; Network Only for mutations and AI routes.
  4. **Install prompt** â€” A bottom banner ("Install Xtimator for quick access") appears after the user has â‰¥1 estimate and has not dismissed it. Stores dismissal in localStorage. Only shown to authenticated users.
  5. **Offline indicator** â€” A subtle top bar "You're offline â€” showing cached data" appears when `navigator.onLine` is false. Create/record CTAs are disabled with tooltip "Requires internet connection".
  6. **Icons** â€” `public/icons/icon-192.png`, `icon-512.png`, `icon-maskable-192.png`, `icon-maskable-512.png` exist; `app/manifest.ts` fallback icons array points to `/icons/*`; `apple-touch-icon` meta tag present in layout.

**Out of scope:** Push notification delivery (â†’ Phase 77), background sync / offline mutation queue, native app store submission.

**Plans:** 2/2 plans complete

Plans:
- [x] 76.1-01-PLAN.md â€” viewport export, apple meta tags, InstallPrompt estimate gate + iOS branch, Wave 0 test stubs, app-icons test fix (PWA-01, PWA-04, PWA-06)
- [x] 76.1-02-PLAN.md â€” offline CTA disabling (New Project buttons) + Lighthouse manual QA checkpoint (PWA-02, PWA-03, PWA-05)

### Phase 76.2: Settings & Admin Persistence Fix â€” DB Schema Sync + Full Audit (INSERTED) â€” COMPLETE 2026-05-21
...
**Depends on:** Phase 76 (Price Book CSV Pro â€” stable baseline before schema changes)

**Requirements:** SETTINGS-FIX-01, SETTINGS-FIX-02, SETTINGS-FIX-03, SETTINGS-FIX-04, SETTINGS-FIX-05

**Success Criteria** (what must be TRUE):
  1. **All 6 missing columns exist in Supabase** â€” `digital_signature_enabled` (BOOL), `estimate_terms_enabled` (BOOL), `estimate_terms_text` (TEXT), `email_delivery_enabled` (BOOL), `sms_delivery_enabled` (BOOL), `ai_model_override` (TEXT) â€” verified via `SELECT column_name FROM information_schema.columns WHERE table_name = 'companies'`.
  2. **`database.types.ts` reflects all columns** â€” all 6 columns present in `Database['public']['Tables']['companies']['Row']` type after regeneration via Supabase MCP or CLI.
  3. **No TypeScript errors in settings/admin actions** â€” `npx tsc --noEmit` exits 0; no implicit `any` or missing-property errors in `lib/actions/settings.ts`, `app/admin/companies/actions.ts`, and all affected form components.
  4. **Server action errors surface visibly** â€” `updateDeliverySettings`, `updateEstimateTerms`, `setCompanyModelOverride` log and return `{ ok: false, error }` on DB failure instead of silently swallowing errors; toast/alert shown in UI on failure.
  5. **Manual verification passes** â€” for every affected page: fill form â†’ save â†’ hard refresh â†’ values match what was saved. Pages: `/settings/delivery`, `/settings/estimate-templates`, `/admin/companies/[id]`. No regression on other settings pages.

**Affected files:**
- `supabase/migrations/` â€” 3 migration files to apply: `20260519000002`, `20260519000003`, `20260520000001`
- `types/database.types.ts` â€” regenerate after applying migrations
- `lib/actions/settings.ts` â€” fix TypeScript errors, surface errors in `updateDeliverySettings` + `updateEstimateTerms`
- `app/admin/companies/actions.ts` â€” fix TypeScript errors in `setCompanyModelOverride`
- `components/settings/delivery-settings-form.tsx` â€” fix type errors after regeneration
- `components/settings/estimate-terms-form.tsx` â€” fix type errors after regeneration
...
Plans:
- [x] 76.2-01-PLAN.md â€” Apply 3 pending migrations + regenerate types + add console.error logging + manual persistence verification (SETTINGS-FIX-01..05)

### Phase 77: Notifications System â€” Unified In-App + Email + (later) Push
...
**Depends on:** Phase 7 (Resend email infra) Â· Phase 67 (Inngest background dispatch) Â· Phase 70 (Stripe payment webhooks as event source) Â· Phase 71 (UI design system)

**Requirements:** NOTIF-01, NOTIF-02, NOTIF-03, NOTIF-04, NOTIF-05, NOTIF-06, NOTIF-07, NOTIF-08, NOTIF-09, NOTIF-10, NOTIF-11, NOTIF-12

**Success Criteria** (what must be TRUE):
  1. **`notifications` table** ships with: id Â· company_id Â· user_id (nullable for company-wide) Â· event_type (enum) Â· title Â· body Â· link_url Â· resource_type Â· resource_id Â· metadata JSONB Â· read_at Â· created_at Â· expires_at. RLS scoped to company_id.
  2. **`notification_preferences` per-user table** â€” JSONB `categories: {estimate: {in_app, email}, payment: {...}, trial: {...}, admin: {...}, whatsapp: {...}}`. Defaults: all in_app=true, email=true except quiet ones.
  3. **`notify()` server-side helper** at `lib/notifications/dispatch.ts` â€” single API for any code path to enqueue a notification. Takes event_type + payload, looks up user preferences, fans out to: insert in_app row + queue email via Resend + (future) push.
  4. **17 event types instrumented** across the app: estimate.viewed Â· estimate.accepted Â· estimate.declined Â· payment.received (Phase 70) Â· trial.expiring_3d Â· trial.expired Â· quota.80pct Â· quota.exhausted Â· whatsapp.inbound Â· ai_job.failed Â· ai_job.completed Â· admin.tier_changed Â· admin.bonus_credits_granted Â· price_book.imported Â· custom_domain.verified Â· invite.accepted Â· system.maintenance
  ...
  7. **Email digest mode** â€” instead of one email per event, group same-category events into a single email if >3 events in 1 hour. Cron via Inngest.
  ...
  9. **Browser push notifications** (Phase 1: scaffold only â€” request permission button + Web Push API service worker registration; actual push delivery deferred to Phase 2 if user signals demand). Adds `push_subscription` JSONB to `notification_preferences`.
  10. **Auto-cleanup cron** runs daily, deletes notifications older than 60 days unless `pinned=true`.
  11. **Real-time updates** via Supabase Realtime â€” `notifications` INSERT subscription on the client increments the bell badge live without refresh.
  12. **Tests** â€” unit: dispatch() preference fan-out (â‰¥10 cases), category filtering (â‰¥6 cases), email digest grouping (â‰¥4 cases). Playwright E2E: walk full flow from trigger event â†’ bell badge updates â†’ click â†’ read marked â†’ navigate.
...
### Phase 78: Admin OG Image Upload â€” File Upload with Preview Feedback
...
**Depends on:** Phase 71 (design system) Â· Phase 66 (storage abstraction)
...
**Success Criteria** (what must be TRUE):
  ...
  2. On file select, show preview of the image in a 1200Ã—630 aspect ratio frame (the actual OG card dimensions) with overlay text "1200Ã—630 recommended". Display detected dimensions; warn if <600Ã—315 with red text, but allow override.
  ...
  4. Remove button clears `og_image_url` to null AND deletes the previous storage object (best-effort â€” don't fail save if delete errors). Confirmation modal before remove.
...
### Phase 79: Estimate Editing Workflow â€” Draft, Consolidate, Float Save & AI Refinement (SEED-028)
...
**Plans:** 4/4 plans complete (Phases Aâ€“D via commits 2f8cb47, cef25df, 974f1e2, 0462b6f â€” executed 2026-05-20)

### Phase 80: Walkthrough Audit, Debug & Polish â€” Tour & Tooltips QA Round 2 (SEED-029)
...
**Success Criteria** (what must be TRUE):
  1. **Diagnosis complete** â€” `WALKTHROUGH-FINDINGS.md` produced after running `tour-uat-runbook.md` in EN/PT/ES on desktop + mobile (390px viewport); every bug listed with screenshot/description.
  2. **Dual selector fixed** â€” `findVisibleTarget` in `tour-spotlight.tsx` resolves the correct (visible) language-toggle element on both topbar and bottom-nav; mobile spotlight lands on the right element.
  3. **A11y hardened** â€” `inert` or equivalent on background content during spotlight step; focus trapped inside the step card; `prefers-reduced-transparency` gate consistent across `TourSpotlight` and `ContextualTooltip`.
  4. **Performance** â€” rAF continuous loop replaced with `ResizeObserver` + scroll listener (or Floating UI `autoUpdate`); no jank on mid-range mobile.
  5. **Telemetry + tests** â€” `tour_started`, `tour_step_completed`, `tour_finished`, `tour_skipped` events logged; 15 Playwright tests in `tour-flow.spec.ts` un-skipped and passing.
...
- [x] 80-03-PLAN.md — A11y (inert) + performance (rAF->autoUpdate) + reduced-transparency consistency
...
**Context:** Inngest is open source (github.com/inngest/inngest) and fully self-hostable. Current cloud free tier (50k runs/month) is sufficient while traffic is low. Migration is triggered by the Hetzner infrastructure move, not independently. All inngest.send() call sites and event schemas remain unchanged — only the server URL and keys change.
