...
- [ ] **Phase 159: Inbox Visual Redesign — "Premium Xtimator"** — Apply the existing Phase-71 glassmorphism design system (`--glass-*` CSS vars, `Card variant="glass"`, `Avatar`/`AvatarFallback` primitives) to the v4.16 Inbox: deterministic-color initials avatars per contact (new utility — same contact always renders the same color), glass-surface list rows + thread pane (replacing the flat `bg-muted`/plain-border treatment), a visually rich unread state (colored accent bar/dot, not just a text `Badge`), and the same glass treatment applied to the Inbox Settings sub-page (`/admin/inbox/settings`, Accounts + Templates tables) for consistency. No dependency — independent of the other 3 phases, only touches `app/admin/inbox/*`. Sequenced last as the largest/most subjective single effort. (INBOX-05, INBOX-06, INBOX-07, INBOX-08)
