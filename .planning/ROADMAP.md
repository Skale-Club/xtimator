...
**Plans**: 2 plans
Plans:
- [ ] 137-01-PLAN.md — acceptInvite(token) server action: service-client token lookup, single-use atomic flip, expiry + email-match enforced, idempotent company_members insert with invite.role, switchActiveCompany (TDD)
- [ ] 137-02-PLAN.md — /invite/accept route (authed join / unauthed signup-with-token) + signUp/signIn ?next=/invite/accept redirect so an invited new user JOINs and SKIPS company creation (test proves no /onboarding)
