...
- 🚧 **v4.16 Admin Inbox Consolidation** — Phases 154-155 (roadmap created 2026-07-05) — consolidate the three scattered super-admin WhatsApp surfaces (`/admin/whatsapp` conversations+accounts, `/admin/whatsapp-templates`, `/admin/integrations/whatsapp` credentials) into one coherent **Inbox**: a single nav item (`/admin/inbox`, old routes redirect), a two-pane master-detail conversation viewer (list + thread on the same page, Xphere-style, replacing the `Sheet` drawer), and an Inbox Settings area folding in Accounts + Templates. Read-only (no reply/send — deferred to INBOXX-01); credentials stay in Integrations; the data layer + DB tables keep their `whatsapp_*` names (user-facing rename only). Phase 154 = route consolidation + settings + all test updates (structural, preserves the old UI); Phase 155 = the master-detail viewer refactor in place. Design reference: the Xphere inbox (`C:\Users\Vanildo\Dev\xphere`)
...
## 🚧 v4.16 Admin Inbox Consolidation (Phases 154-155) — ROADMAP CREATED 2026-07-05
...
> **Numbering:** continues the GLOBAL phase counter. v4.15 ended at Phase 153. **v4.16 starts at Phase 154.** Do NOT reset to 1. This is NOT a `--reset-phase-numbers` run. (The `gsd-tools init` `current_milestone`/`latest_completed_milestone` fields are known-stale — trust the ROADMAP counter: last formal phase was 153.)
>
...
>
> **Locked scope guardrails (do NOT plan against):** Read-only Inbox — visualize conversations, NEVER reply/send (preserves the deliberate WAADM read-only posture; two-way replies are deferred to INBOXX-01). Inbox = operations (conversations + Accounts + Templates); Integrations = credentials — the raw Meta credentials (API token + Phone Number ID + WABA ID + Display Number + AI system prompt) STAY in `/admin/integrations/whatsapp`, unchanged. Layout = two-pane master-detail (persistent right thread pane), NOT a modal drawer. User-facing rename ONLY — slug + nav label + page copy become "Inbox"; the data layer (`lib/queries/admin-whatsapp.ts`, `lib/actions/admin-whatsapp*.ts`) and DB tables (`whatsapp_*`) keep their WhatsApp names (channel-specific; "Inbox" is the forward-looking multi-channel container — keeps the blast radius small). Reuse, do NOT rebuild — reuse `listAdminWhatsAppConversations`/`parseAdminWhatsAppFilters` (list + filters + pagination), `loadAdminConversationThread` (thread + 30-day history + signed media), `MessageBubble`, `AdminWhatsAppAccounts`, `WhatsAppTemplatesPanel`. Tests ship in the same change — renaming `/admin/whatsapp` breaks path/existence assertions in several unit + e2e files; those updates are part of the milestone, not follow-up. Tenant WhatsApp surfaces (tombstoned tenant page, notification-channel toggle) are OUT of scope.
...
**Success Criteria** (what must be TRUE):
  ...
  1. The super-admin left-nav shows a single **Inbox** item linking to `/admin/inbox` (lucide `Inbox` icon) in place of the two separate "WhatsApp" (`/admin/whatsapp`) and "WA Templates" (`/admin/whatsapp-templates`) items; navigating there renders the conversation surface (the existing table + `Sheet` thread UI is preserved verbatim in this phase — no visual refactor yet)
  2. Visiting the old `/admin/whatsapp` redirects to `/admin/inbox` and the old `/admin/whatsapp-templates` redirects to `/admin/inbox/settings` (no broken bookmarks); every hardcoded `/admin/whatsapp` / `/admin/whatsapp-templates` reference (nav, pagination URLs, filter `router.replace`, `revalidatePath` in the account actions) is retargeted, and a grep guard finds no remaining literal outside the redirect stubs
  3. A gear/"Settings" affordance in the Inbox header opens `/admin/inbox/settings` — a `requireAdmin`-gated tabbed page with an **Accounts** tab (the existing company-config + authorized-sender provisioning UI, `AdminWhatsAppAccounts`) and a **Templates** tab (the existing Meta-template builder, `WhatsAppTemplatesPanel`), both reusing the current components unchanged; a back affordance returns to `/admin/inbox`, and `revalidatePath` for account mutations targets the new settings route
...
**Depends on**: Phase 154 (the `/admin/inbox` route + nav + settings + green tests must be established first; this phase upgrades the UI in place, so the structural layer is green before the visual refactor — minimizing risk).
**Requirements**: INBOX-02
**Success Criteria** (what must be TRUE):

  1. `/admin/inbox` renders a two-pane master-detail layout — a scrollable conversation list on the LEFT (Xphere-style rows: contact name semibold, last-message preview truncated/muted, timestamp, unread indicator, hover + selected states, company name as a secondary label) with the existing search/filters and server-side pagination, and the conversation thread on the RIGHT pane on the SAME page — with the old table + right-side `Sheet` overlay fully removed
  2. Selecting a conversation updates `?conversation=<id>` (shallow, list stays mounted) and loads its thread via the reused `loadAdminConversationThread` + `MessageBubble` (read-only, with the "Read-only. Shows up to the last 30 days of messages." note); no modal/drawer opens
  3. A direct link / refresh to `/admin/inbox?conversation=<id>` SSR-selects and renders that thread on load; with no `?conversation=`, an empty-state prompts the admin to pick a conversation
...
**Plans:** 2/2 plans complete
Plans:
- [x] 155-01-PLAN.md — Two-pane master-detail refactor of admin-whatsapp-client.tsx + page.tsx wiring (URL-driven selection, SSR deep-link, empty state, mobile collapse)
- [x] 155-02-PLAN.md — Close the 6 Wave-0 test gaps (stale static-contract fix, row-click/direct-link/empty-state/mobile-collapse/no-reply-send tests)
**UI hint**: yes
