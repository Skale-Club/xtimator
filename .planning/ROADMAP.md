...
- 🚧 **v4.18 Estimate Document & Send Experience Refresh** — Phases 160-163 (roadmap created 2026-07-08) — a per-estimate settings panel (gear button on the floating `Photos / Send` pill) for tax/discount/deposit overrides + non-destructive section visibility, applied consistently across editor/share/PDF/plain-text/WhatsApp via one shared resolver; a format-first Send hub (Online Estimate/PDF/Plain Text) replacing the channel-first Email/SMS tabs + friendlier branded estimate URLs (`/estimate/{companySlug}/{estimateSlug}-{shortToken}`, old token links keep working forever, no new anon RLS ever added to `estimates`); a mobile line-item editor rebuilt to match the desktop document-native table language; and a full document alignment pass + inline-editable Bill To with a consolidated client-picker (today duplicated 4x). Server-side math stays authoritative via `lib/estimate/compute-totals.ts` throughout — GUARD-03 unchanged. SEED-041 + SEED-042 + SEED-043 + SEED-044.
...
**Plans:** 5 plans
Plans:
- [ ] 162-01-PLAN.md — Wave 0 test scaffolding (7 test files, 63 it.todo markers) unblocking RED->GREEN cycles for DOCUX-01..07
- [ ] 162-02-PLAN.md — Consolidated ClientPicker (4 variants) + call-site swap; delete link-client-{button,card}.tsx; DocumentClient.id plumbing (DOCUX-03)
- [ ] 162-03-PLAN.md — Bill To pencil affordance + InlineProjectName reconciliation + full alignment pass (SECTION_PX + unified vertical rhythm) + regenerated share visual baselines (DOCUX-02, DOCUX-04, DOCUX-05)
