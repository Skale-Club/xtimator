...
| 142. Monthly Credit Grant Decouple | v4.13 | 0/1 | Planned     | - |
...
**Plans**: 1 plan
- [ ] 142-01-PLAN.md — add shared monthGrantKey(companyId,date) helper + re-key the invoice.paid grant from event.id to the company-month key; add + register the Inngest monthly-credit-grant cron ('0 5 1 * *') granting active paying companies via the idempotent grantCredits on the SAME key; load-bearing no-double-grant regression test (ANN-02)
