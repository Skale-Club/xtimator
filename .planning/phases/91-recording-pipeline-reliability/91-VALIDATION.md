...
| 91-02 T1 | 02 | 2 | REC-04, REC-03 | unit (Wave 0, RED) | `npx vitest run tests/unit/inngest/transcribe-audio-job.test.ts tests/unit/capture/capture-attempt-lineage.test.ts` | ⚠️ transcribe exists (confirm OpenRouter path + add dedup assertion); lineage NEW | ⬜ pending |
...
| 91-02 T4 | 02 | 2 | REC-05 (consumer half) | tsc + full unit | `npx tsc --noEmit && npx vitest run` | ✅ (rewires 3 prod callers; tsc validates the union after `as` casts removed) | ⬜ pending |
...
- [ ] **(Plan 02 T1)** `tests/unit/inngest/transcribe-audio-job.test.ts` — CONFIRM the suite already targets the OpenRouter/`transcribeAudioOR` path + `step.run('whisper-transcribe')` + `idempotency: 'event.data.recordingId'` (it does — no legacy OpenAI-URL job assertion to remove; the line-47 `api.openai.com` match is the correct CLIENT-module assertion). ADD the genuinely-new dedup-intent assertion: recordingId idempotency key co-exists with the `step.run('whisper-transcribe')` boundary so an already-successful transcription is not re-charged on Retry. (REC-04)
...
| Non-capture pollJob consumers surface failures (no false success) | REC-05 | Requires real browser flows for text-describe / photos-input / header AI input | With Inngest unconfigured: run text-describe + photos-input + the header AI input; confirm each shows a failure toast and does NOT navigate as if successful; the transcription flow does not proceed to generation on a failed transcript |
