...
provides:
  ...
  - Zero hardcoded "EstimateBuilder Pro" strings in app/components/lib (ADMIN-07)
...
**All 5 provider-key env reads migrated to `getIntegrationKey()` with graceful 503/error responses on null; all 5 remaining "EstimateBuilder Pro" hardcoded strings replaced with `getBranding()` loader calls; two grep-assertion unit tests enforce future compliance; e2e auth assertion made env-driven via `APP_NAME_E2E`. Phase 8 is now feature-complete.**
...
- **5 branding sites migrated:**
  ...
  - `components/share/estimate-view.tsx` — Added `appName: string` prop; footer "Powered by EstimateBuilder Pro" → "Generated by {appName}".
...
- **`tests/unit/platform-branding-sweep.test.ts`** — Filesystem walk across same roots asserting zero `EstimateBuilder Pro` literals in any `.ts`/`.tsx` source file. Passes 1/1.
...
- `grep -rn "EstimateBuilder Pro" app/ components/ lib/` → 0 matches ✓
...
- `grep -c "EstimateBuilder Pro" tests/e2e/auth.spec.ts` → 0 ✓
