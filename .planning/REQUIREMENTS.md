...
- [ ] **DOMAIN-01**: Owner can enter and save a custom domain for their company from the settings page
...
- [ ] **DOMAIN-03**: A request arriving at a custom domain (e.g., `estimates.mycompany.com/estimate/{token}`) renders the correct estimate without redirecting to xtimator.com and without requiring authentication

### White-label

- [ ] **DOMAIN-04**: When the estimate is served from a custom domain, the "Generated by Xtimator" footer is hidden and only the company's own branding (logo, name, color) is shown
- [ ] **DOMAIN-05**: Companies without a custom domain configured continue to receive share links on xtimator.com — no regression in existing behavior
