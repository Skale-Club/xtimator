...
- [x] **PUBURL-01**: A shared estimate can be opened via a friendly branded URL shaped `/estimate/{companySlug}/{estimateSlug}-{shortToken}`, generated for every estimate (existing and new). *(Complete — Plan 01 landed the schema/builder, Plan 03 landed the route that resolves it. Plan 05's new-estimate wiring/backfill ensures every row carries a token, file-disjoint from this requirement's route-resolution proof.)*
