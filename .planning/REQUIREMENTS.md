...
- [x] **PROV-02**: An automated check fails the build if a server-side module reintroduces a hardcoded Supabase-only storage path, keeping the provider seam from silently rotting the way it did after Phase 66.
- [x] **PROV-03**: With R2 configured, the WhatsApp inbound media path (the concrete case that would have broken) writes and reads the same backend end-to-end.
...
- [x] **MIG-04**: The runbook documents the cutover and the rollback, states the verified R2 settings, and contains no real secrets.
